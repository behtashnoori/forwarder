...
from backend.services.case_document_service import DocumentError, FORMAT_CATALOG, detect_format
...
def upload(shipment: OperationalShipment, actor_id: int, file: FileStorage, title: str, description: str | None, key: str, replacement: CaseDocumentFile | None = None) -> CaseDocumentFile:
    title = str(title or "").strip()
    if not title:
        raise DocumentError("نوع یا عنوان تجاری سند الزامی است")
    if not key or len(key) > 100:
        raise DocumentError("شناسه امن تلاش بارگذاری الزامی است")
    original = secure_filename(Path(file.filename or "").name)
    ...
    try:
        ...
        if replacement is not None and (replacement.owner_type != "SHIPMENT" or replacement.operational_shipment_id != shipment.id or replacement.status != "active"):
            raise DocumentError("نسخه فعال انتخاب‌شده برای جایگزینی در دسترس نیست", 409)
        version = replacement.version_number + 1 if replacement is not None else 1
        ...
        if replacement is not None:
            replacement.status, replacement.superseded_at, replacement.superseded_by = "superseded", datetime.utcnow(), row.id
        ...
        db.session.add(OperationalAudit(organization_id=shipment.organization_id, actor_user_id=actor_id,
            ...
            metadata_json={"shipment_id": shipment.id, "owner": "SHIPMENT", "version": version,
                           "supersedes_document_public_id": replacement.public_id if replacement else None}))
        db.session.commit()
        return row
...
def remove(shipment: OperationalShipment, row: CaseDocumentFile, actor_id: int, reason: str):
    if row.status == "deleted":
        raise DocumentError("سند حذف‌شده در دسترس نیست", 404)
