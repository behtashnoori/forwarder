...
def customer_app():
    ...
    with app.app_context():
        ...
        manager_a = ExpertUser(username="customer-manager-a", password_hash="x", full_name="Manager A", role="admin", authority="ORGANIZATION_ADMIN", is_active=True)
        manager_b = ExpertUser(username="customer-manager-b", password_hash="x", full_name="Manager B", role="admin", authority="ORGANIZATION_ADMIN", is_active=True)
        ...
        db.session.add_all([org_a, org_b, manager_a, manager_b, platform, ordinary, origin, destination]); db.session.flush()
        ...
        tokens = {name: create_session_tokens(user.id)["access_token"] for name, user in {"a": manager_a, "b": manager_b, "platform": platform, "ordinary": ordinary}.items()}
        yield {"app": app, "a": manager_a.id, "org_a": org_a.id, "foreign": foreign.id, "origin": origin.id, "destination": destination.id, "tokens": tokens}
...
def test_tenant_customer_create_list_search_selector_and_direct_roundtrip(customer_app):
    ...
    selector = client.get("/api/operations/selectors/customers?q=ایران خودرو", headers=headers(customer_app["tokens"]["a"]))
