...
def app():
    ...
    with app.app_context():
        ...
        hamburg = InternationalCity(
            name_en="Hamburg", name_fa="Hamburg", country_id=de.id
        )
        port = IranPort(
            code="BND",
            name_en="Bandar",
        ...
        customs = CustomsOffice(
            ...
            name_en="Border",
            name_fa="Duplicate",
            customs_type="road_border",
            country_id=iran.id,
            province_id=province.id,
            county_id=county.id,
            city_id=city.id,
        )
        incomplete = CustomsOffice(
            code="BAD",
            name_en="Bad",
            name_fa="Bad",
            customs_type="road_border",
        ...
        db.session.add_all(
            [city, duplicate, inactive, hamburg, port, customs, incomplete]
        )
        db.session.flush()
        db.session.add(
            PortLocation(
        ...
        app.config["geo"] = {
            ...
            "other_province": other.id,
            "city": city.id,
            "duplicate_city": duplicate.id,
...
def test_resolver_covers_supported_types_and_ancestry(app):
    with app.app_context():
        ...
        assert (
            resolve_location(
                {"source_type": "city", "source_id": ids["city"]}
            ).province_id
            == ids["province"]
        )
        assert (
            resolve_location(
                {"source_type": "international_city", "source_id": ids["hamburg"]}
            ).country_code
            == "DE"
        )
        assert (
            resolve_location(
                {"source_type": "iran_port", "source_id": ids["port"]}
            ).location_type
            == "port"
        )
        assert (
            resolve_location(
                {"source_type": "customs_office", "source_id": ids["customs"]}
            ).county_name
            == "Tehran County"
        )
...
def test_iran_projection_is_eligible_typed_and_disambiguated(app):
    ...
    assert [
        (row["identity"]["type"], row["identity"]["id"]) for row in port.json["data"]
    ] == [
...
def test_request_round_trip_uses_ids_derives_iran_province_and_keeps_legacy_reads(app):
    ...
    with app.app_context():
        ...
        assert (row.origin_country_id, row.origin_international_city_id) == (
            app.config["geo"]["DE"],
            app.config["geo"]["hamburg"],
        )
        assert (
            row.dest_country_id == app.config["geo"]["IR"]
            and row.dest_international_city_id is None
        )
        assert (
            row.iran_dest_city_id == app.config["geo"]["city"]
            and row.iran_entry_province_id == app.config["geo"]["province"]
        )
        assert build_route_payload(row)["location_state"] == "canonical"
        legacy = ShipmentRequest(
            shipping_type="international",
            contact_phone="09111111111",
            origin_country="Old",
            origin_city_international="Text",
            dest_country="Old",
            dest_city_international="Text",
        )
    ...
    with app.app_context():
        ...
        assert (
            row.dest_country_id,
            row.dest_international_city_id,
            row.dest_country,
            row.dest_city_international,
        ) == (
            app.config["geo"]["DE"],
            app.config["geo"]["hamburg"],
            "Germany",
...
def test_iran_origin_requires_province_and_rejects_lower_mismatch(app):
    ...
    assert (
        ...
        and missing.json["error"]["code"] == "LOCATION_MAPPING_REQUIRED"
    )
    valid = app.test_client().post(
        ...
        json={
            **base,
            ...
            "origin_location": {"type": "city", "id": ids["city"]},
        },
    )
    ...
    assert (
        ...
        and mismatch.json["error"]["code"] == "LOCATION_ANCESTRY_MISMATCH"
    )
    redundant = app.test_client().post(
        "/api/shipment-request",
        json={
            **_payload(app),
    ...
    assert (
        redundant.status_code == 400
        and redundant.json["error"]["code"] == "VALIDATION_FAILED"
    )
    ...
    with app.app_context():
        ...
        payload = {
            ...
            "origin": {
                ...
                "source_id": ids["hamburg"],
            },
            "destination": {
                "source_type": "customs_office",
                "source_id": ids["customs"],
            },
            "transport_mode": "road",
            "planned_departure": "2026-08-10T10:00:00Z",
            "planned_arrival": "2026-08-10T12:00:00Z",
        }
        shipment, created = operational_service.create_from_accepted_quote(
            payload, {"id": user.id}, "slice3"
        )
        leg = RouteLeg.query.one()
        assert created and shipment.source_type == "accepted_quote"
        assert leg.destination_snapshot["province"] == {
            "id": ids["province"],
            "name": "Tehran",
        }
        assert leg.destination_snapshot["canonical_reference"] == {
            "source_type": "customs_office",
            "source_id": ids["customs"],
        }
