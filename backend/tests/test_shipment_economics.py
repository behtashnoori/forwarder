...
def app():
    ...
    with app.app_context():
        ...
        app.config["econ"]={"user":{"id":user.id},"outsider":{"id":outsider.id},"shipment":shipment.public_id,"other_shipment":other_shipment.public_id,"service":service.id}
        yield app;db.session.remove();db.drop_all()

def command(app,side,stage,amount,currency="USD",key="key"):
    return {"side":side,"stage":stage,"service_type_id":app.config["econ"]["service"],"money":{"amount":amount,"currency":currency},"effective_at":datetime.now(timezone.utc).isoformat(),"authority":"FE-2 test authority","source_type":"MANUAL_TEST","reason":"test fact","idempotency_key":key}
...
def test_fx_is_explicit_and_missing_fx_abstains(app):
    with app.app_context():
        ctx=app.config["econ"];economics.create_line(ctx["shipment"],command(app,"REVENUE","ESTIMATE","100","USD","fx-rev"),ctx["user"]);economics.create_line(ctx["shipment"],command(app,"COST","ESTIMATE","50","EUR","fx-cost"),ctx["user"])
        ...
        complete=economics.projection(ctx["shipment"],ctx["user"],"USD")["stages"]["ESTIMATE"]
...
def test_quote_requires_explicit_preview_and_confirm(app):
    with app.app_context():
        ...
        result=economics.quote_confirm(ctx["shipment"],{"service_type_id":ctx["service"],"authority":"accepted customer capability response","reason":"authorized commercial acceptance","idempotency_key":"quote-confirm"},ctx["user"])
