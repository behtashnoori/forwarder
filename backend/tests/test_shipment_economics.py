...
def app():
    ...
    with app.app_context():
        ...
        shipment=OperationalShipment(organization_id=org.id,shipment_request_id=req.id,accepted_quote_id=quote.id,lifecycle_status="planned",created_by_user_id=user.id,primary_responsible_expert_id=user.id);other_req=ShipmentRequest(shipping_type="domestic",contact_phone="09121111111");db.session.add_all([shipment,other_req]);db.session.flush()
        other_quote=ExpertQuote(shipment_request_id=other_req.id,amount=50,currency="USD",created_by_expert_id=outsider.id,customer_response="accepted",responded_at=datetime.now(timezone.utc));db.session.add(other_quote);db.session.flush();other_shipment=OperationalShipment(organization_id=other.id,shipment_request_id=other_req.id,accepted_quote_id=other_quote.id,lifecycle_status="planned",created_by_user_id=outsider.id,primary_responsible_expert_id=outsider.id);db.session.add(other_shipment);db.session.commit()
