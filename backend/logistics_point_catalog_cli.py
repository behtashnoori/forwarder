...
from backend.logistics_point_catalog import (
    LogisticsCatalogError,
    apply_catalog,
    load_catalog,
    plan_catalog,
)
...
def run(argv=None):
    try:
        return main(argv)
    except LogisticsCatalogError as exc:
        print(f"Logistics catalog command failed: {exc}", file=sys.stderr)
        return 1
    except Exception as exc:
        print(
            f"Logistics catalog command failed ({type(exc).__name__}).", file=sys.stderr
        )
        return 1
