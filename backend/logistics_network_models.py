...
PROJECT_LOGISTICS_ROLES = frozenset({
    "ORIGIN", "INTERMEDIATE", "DESTINATION", "CUSTOMS_PROCESSING",
    "TRANSFER", "STORAGE", "LOADING", "UNLOADING", "OTHER_GOVERNED",
})
...
class LogisticsPointType(db.Model):
    ...
    __table_args__ = (
        ...
        db.CheckConstraint("version >= 1", name="ck_logistics_point_type_version_positive"),
...
class LogisticsPoint(db.Model):
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint("organization_id", "immutable_code", name="uq_logistics_point_org_code"),
        db.UniqueConstraint("id", "organization_id", name="uq_logistics_point_id_org"),
        db.UniqueConstraint(
            "organization_id", "normalized_name", "logistics_point_type_id", "country_id",
            "geography_key", name="uq_logistics_point_exact_duplicate",
        ),
    ...
    immutable_code = db.Column(db.String(64), nullable=False)
    logistics_point_type_id = db.Column(BIGINT, db.ForeignKey("logistics_point_type.id", ondelete="RESTRICT"), nullable=False)
    fa_name = db.Column(db.String(160), nullable=False)
    normalized_name = db.Column(db.String(200), nullable=False)
    en_name = db.Column(db.String(160), nullable=True)
    country_id = db.Column(BIGINT, db.ForeignKey("country.id", ondelete="RESTRICT"), nullable=False)
    province_id = db.Column(BIGINT, db.ForeignKey("province.id", ondelete="RESTRICT"), nullable=True)
    city_id = db.Column(BIGINT, db.ForeignKey("city.id", ondelete="RESTRICT"), nullable=True)
...
class ProjectLogisticsPoint(db.Model):
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint("project_id", "logistics_point_id", "project_role", name="uq_project_logistics_point_role"),
        db.ForeignKeyConstraint(["project_id", "organization_id"], ["project.id", "project.organization_id"], name="fk_project_logistics_point_project_org", ondelete="RESTRICT"),
        db.ForeignKeyConstraint(["logistics_point_id", "organization_id"], ["logistics_point.id", "logistics_point.organization_id"], name="fk_project_logistics_point_point_org", ondelete="RESTRICT"),
        db.CheckConstraint("sequence_number >= 1", name="ck_project_logistics_point_sequence_positive"),
        db.CheckConstraint("project_role IN ('ORIGIN','INTERMEDIATE','DESTINATION','CUSTOMS_PROCESSING','TRANSFER','STORAGE','LOADING','UNLOADING','OTHER_GOVERNED')", name="ck_project_logistics_point_role"),
        db.CheckConstraint("version >= 1", name="ck_project_logistics_point_version_positive"),
        db.Index("uq_project_logistics_point_active_sequence", "project_id", "sequence_number", unique=True, postgresql_where=db.text("is_active"), sqlite_where=db.text("is_active = 1")),
        db.Index("ix_project_logistics_point_project_active", "project_id", "is_active", "sequence_number"),
    )
    id = db.Column(BIGINT, primary_key=True)
    public_id = db.Column(db.String(36), nullable=False, default=lambda: str(uuid4()))
