...
def _error(exc: service.OperationalError):
    return jsonify({"error": {"code": exc.code, "message": exc.message, "fields": []}}), exc.status


def _user():
    user = get_current_user()
    if not user:
        raise service.OperationalError("FORBIDDEN_OPERATION", "Authentication is required.", 401)
    return user
...
def create_from_quote():
    try:
        shipment, created = service.create_from_accepted_quote(request.get_json(silent=True) or {}, _user(), request.headers.get("Idempotency-Key", ""))
        return jsonify({"data": service.shipment_graph(shipment), "meta": {"created": created}}), 201 if created else 200
    except service.OperationalError as exc:
...
def list_shipments():
    try:
        user=_user(); service.require_permission(user, "operational_shipment.read"); org=service.organization_for_user(user["id"])
        page=max(1, request.args.get("page", 1, type=int)); per_page=min(100, max(1, request.args.get("per_page", 20, type=int)))
        query=select(OperationalShipment).where(OperationalShipment.organization_id == org)
        if request.args.get("status"): query=query.where(OperationalShipment.lifecycle_status == request.args["status"])
        customer=request.args.get("customer","").strip()
        if customer:
            pattern=f"%{customer}%"; query=query.join(ShipmentRequest, ShipmentRequest.id == OperationalShipment.shipment_request_id).where(or_(ShipmentRequest.customer_first_name.ilike(pattern),ShipmentRequest.customer_last_name.ilike(pattern),ShipmentRequest.contact_phone.ilike(pattern)))
        if request.args.get("request_id", type=int): query=query.where(OperationalShipment.shipment_request_id == request.args.get("request_id", type=int))
        if request.args.get("accepted_quote_id", type=int): query=query.where(OperationalShipment.accepted_quote_id == request.args.get("accepted_quote_id", type=int))
        query=query.join(RoutePlan, (RoutePlan.operational_shipment_id == OperationalShipment.id) & RoutePlan.is_active.is_(True)).join(RouteLeg, RouteLeg.route_plan_id == RoutePlan.id)
        if request.args.get("origin"): query=query.where(RouteLeg.origin_snapshot.cast(db.String).ilike(f"%{request.args['origin'].strip()}%"))
        if request.args.get("destination"): query=query.where(RouteLeg.destination_snapshot.cast(db.String).ilike(f"%{request.args['destination'].strip()}%"))
        for name,column in (("date_from",RouteLeg.planned_departure),("date_to",RouteLeg.planned_arrival)):
            value=request.args.get(name)
            if value:
                try: query=query.where(column >= datetime.fromisoformat(value.replace("Z","+00:00")) if name == "date_from" else column <= datetime.fromisoformat(value.replace("Z","+00:00")))
                except ValueError: raise service.OperationalError("INVALID_ROUTE_TIMELINE", f"{name} must be ISO-8601.")
...
@operations_bp.get("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/validate")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/activate")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/replan")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/legs")
@require_auth
...
@operations_bp.patch("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/legs/<int:leg_id>")
@require_auth
...
@operations_bp.delete("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/legs/<int:leg_id>")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/checkpoints")
@require_auth
...
@operations_bp.patch("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/checkpoints/<int:checkpoint_id>")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/route-plans/<int:plan_id>/dependencies")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/checkpoints/<int:checkpoint_id>/arrive")
@require_auth
def checkpoint_arrive(shipment_id, checkpoint_id): return _checkpoint_action(shipment_id, checkpoint_id, "arrive")


@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/checkpoints/<int:checkpoint_id>/complete-processing")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/checkpoints/<int:checkpoint_id>/depart")
@require_auth
def checkpoint_depart(shipment_id, checkpoint_id): return _checkpoint_action(shipment_id, checkpoint_id, "depart")


@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/checkpoints/<int:checkpoint_id>/milestones/<int:milestone_id>/verify")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/checkpoints/<int:checkpoint_id>/milestones/<int:milestone_id>/correct")
@require_auth
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/route-exceptions/reconcile")
@require_auth
...
def route_exception_list(shipment_id=None):
    try:
        ...
        if shipment_id is not None:
            ...
            data=[row for row in data if row["shipment_public_id"] == shipment.public_id]
...
@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/milestones/<int:milestone_id>/events")
@require_auth
def record_event(shipment_id: int, milestone_id: int):
    try:
        event=service.record_event(shipment_id, milestone_id, request.get_json(silent=True) or {}, _user(), request.headers.get("Idempotency-Key", "")); return jsonify({"data":{"id":event.id,"event_type":event.event_type}}), 201
    except service.OperationalError as exc: db.session.rollback(); return _error(exc)


@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/milestones/<int:milestone_id>/verify")
@require_auth
def verify(shipment_id: int, milestone_id: int):
    try:
        payload=request.get_json(silent=True) or {}; row=service.verify_milestone(shipment_id, milestone_id, payload.get("expected_version"), _user()); return jsonify({"data":{"id":row.id,"version":row.version,"verification_state":row.verification_state}})
    except service.OperationalError as exc: db.session.rollback(); return _error(exc)


@operations_bp.post("/api/operational-shipments/<uuid:shipment_id>/milestones/<int:milestone_id>/correct")
@require_auth
def correct(shipment_id: int, milestone_id: int):
    try:
        event=service.correct_milestone(shipment_id, milestone_id, request.get_json(silent=True) or {}, _user(), request.headers.get("Idempotency-Key", "")); return jsonify({"data":{"id":event.id,"event_type":event.event_type}}), 201
    except service.OperationalError as exc: db.session.rollback(); return _error(exc)
...
def work_queue():
    try:
        user=_user(); service.require_permission(user, "work_item.read"); org=service.organization_for_user(user["id"])
        page=max(1, request.args.get("page", 1, type=int)); per_page=min(100, max(1, request.args.get("per_page", 20, type=int)))
        status=request.args.get("status", "open"); query=select(OperationalWorkItem).where(OperationalWorkItem.organization_id == org)
        if status: query=query.where(OperationalWorkItem.status == status)
        if request.args.get("work_type"): query=query.where(OperationalWorkItem.work_type == request.args["work_type"])
        ...
        if shipment_public_id:
            ...
            query=query.where(OperationalWorkItem.operational_shipment_id == shipment.id)
        rows=db.session.scalars(query.order_by(OperationalWorkItem.due_at.asc()).offset((page-1)*per_page).limit(per_page+1)).all(); has_more=len(rows)>per_page
        data=[]
...
def resolve_item(item_id: int):
    try:
        item=service.resolve_work_item(item_id, (request.get_json(silent=True) or {}).get("expected_version"), _user()); return jsonify({"data":{"id":item.id,"status":item.status,"version":item.version}})
    except service.OperationalError as exc: db.session.rollback(); return _error(exc)
