...
async function shipment(page: Page) { await page.goto(`/operations/shipments/${fixture.shipment_a}`); await expect(page.getByText("کالا و وسایل حمل")).toBeVisible(); }
...
test("Catalog → operational cargo → Shared Transport → Tracking", async ({page}) => {
  ...
  await expect(cargo).toContainText("12.500000"); await expect(cargo).toContainText("[SHARED-E2E] Customer A");
  const itemId=await cargo.getAttribute("data-cargo-public-id");
  ...
  await page.reload(); await expect(page.getByRole("article").filter({hasText:fixture.catalog_name})).toBeVisible();
  ...
  await shipment(page); await expect(page.getByText("وضعیت و پیگیری حمل")).toBeVisible(); await expect(page.getByRole("article").filter({hasText:fixture.catalog_name}).first()).toContainText("12.500000"); await expect(page.getByRole("article").filter({hasText:"SHARED-E2E-UNIT"})).toContainText(fixture.catalog_name);
  await page.reload(); await expect(page.getByRole("article").filter({hasText:"SHARED-E2E-UNIT"})).toContainText(fixture.catalog_name);
