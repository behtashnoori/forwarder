...
function AssertCandidateProcess($Process,[string]$ExpectedPython){if(-not(SamePath ([string]$Process.ExecutablePath) $ExpectedPython)){Stop-Containment 'listener executable is not candidate runtime'};$line=[string]$Process.CommandLine;if($line-notmatch'(?i)-m\s+waitress'-or$line-notmatch'(?i)--listen=127\.0\.0\.1:5101'-or$line-notmatch'(?i)backend\.wsgi:app'){Stop-Containment 'listener command is not governed Waitress invocation'}}
...
$rows=@(Get-NetTCPConnection -State Listen -LocalPort $BackendPort -ErrorAction SilentlyContinue|Where-Object{$_.LocalAddress-eq'127.0.0.1'});$owners=@($rows|Select-Object -ExpandProperty OwningProcess -Unique);if($owners.Count-gt1){Stop-Containment 'listener ownership ambiguous'}
...
if($owners.Count-eq1){$again=Get-CimInstance Win32_Process -Filter ('ProcessId='+[int]$owners[0]) -ErrorAction SilentlyContinue;if($again){AssertCandidateProcess $again $candidatePython;Stop-Process -Id ([int]$owners[0]) -Force -ErrorAction Stop}}
$remaining=@(Get-NetTCPConnection -State Listen -LocalPort $BackendPort -ErrorAction SilentlyContinue|Where-Object{$_.LocalAddress-eq'127.0.0.1'});if($remaining.Count-ne0){Stop-Containment 'candidate listener remains after scoped stop'}
