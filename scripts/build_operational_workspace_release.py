...
foreach($line in Get-Content -LiteralPath (Join-Path $PackageRoot 'SHA256SUMS.txt')){$parts=$line -split '  ',2;if($parts.Count -ne 2){throw 'VERIFY_FAIL malformed checksum'};$path=Join-Path $PackageRoot $parts[1].Replace('/','\\');if(-not(Test-Path -LiteralPath $path)){throw 'VERIFY_FAIL missing checksummed file '+$parts[1]};if((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $parts[0].ToLowerInvariant()){throw 'VERIFY_FAIL checksum '+$parts[1]}}
Write-Output 'PACKAGE_LAYOUT=PASS';Write-Output 'PACKAGE_CHECKSUMS=PASS';Write-Output 'RUNTIME_VERIFICATION=PASS'
"""
