...
$runtime = Join-Path $runtimeParent "forwarder-p308-owned-$runId"
...
try {
  ...
  if ($head -ne '20261008_phase3_cargo_delivery') { throw "Unexpected migration head: $head" }
  $pgPort = Free-Port
  & (Join-Path $pgBin 'initdb.exe') -D $pgData -U postgres --auth-host=trust --auth-local=trust --encoding=UTF8 *> (Join-Path $EvidenceDirectory 'initdb.log')
  ...
  python -m pytest backend/tests/test_phase3_cargo_delivery_postgresql.py backend/tests/test_phase3_reported_facts_postgresql.py backend/tests/test_customer_entitlement_postgresql.py backend/tests/test_phase3_document_context_postgresql.py -q --disable-warnings *> (Join-Path $EvidenceDirectory 'postgresql.log')
  ...
  if ($IncludeRegressions) {
    ...
    Write-Output 'P3-01 through P3-05 PostgreSQL PASS'
  }
  $journeys = @(@{ name='P308'; seed='cargo_delivery'; spec='cargo-delivery' })
  if ($IncludeRegressions) {
    $journeys += @(
      @{ name='P307'; seed='reported_facts'; spec='reported-facts' },
      @{ name='P306'; seed='document_context'; spec='document-context' },
      @{ name='P305'; seed='cargo_allocation'; spec='cargo-allocation' },
      @{ name='P304'; seed='transport_execution'; spec='transport-execution' },
      @{ name='P303'; seed='branched_route'; spec='branched-route' },
      @{ name='P302'; seed='cargo_lineage'; spec='cargo-lineage' },
  ...
  foreach ($journey in $journeys) {
    ...
    python "scripts/uat/seed_phase3_$($journey.seed)_e2e.py" *> (Join-Path $EvidenceDirectory "$name-seed.log")
    ...
    npx playwright test "e2e/phase3-$($journey.spec).spec.ts" --reporter=line --output (Join-Path $EvidenceDirectory "$name-browser") *> (Join-Path $EvidenceDirectory "$name-browser.log")
  ...
  @{ product_sha=$productHead; dirty_source=$dirty; schema=$head; results=$results.ToArray();
  ...
  if ((Split-Path -Parent $resolvedRuntime) -ne $resolvedParent -or
      (Split-Path -Leaf $resolvedRuntime) -ne "forwarder-p308-owned-$runId") {
