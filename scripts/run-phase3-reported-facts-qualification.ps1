...
$runtime = Join-Path $runtimeParent "forwarder-p307-owned-$runId"
...
try {
  ...
  if ($head -ne '20261007_phase3_reported_facts') { throw "Unexpected migration head: $head" }
  $pgPort = Free-Port
  ...
  python -m pytest backend/tests/test_phase3_reported_facts_postgresql.py backend/tests/test_customer_entitlement_postgresql.py backend/tests/test_phase3_document_context_postgresql.py -q --disable-warnings *> (Join-Path $EvidenceDirectory 'postgresql.log')
  ...
  $journeys = @(@{ name='P307'; seed='reported_facts'; spec='reported-facts' })
  if ($IncludeRegressions) {
    $journeys += @(
  ...
  if ((Split-Path -Parent $resolvedRuntime) -ne $resolvedParent -or
      (Split-Path -Leaf $resolvedRuntime) -ne "forwarder-p307-owned-$runId") {
