...
def pwsh(): return shutil.which("powershell")
...
def test_packaged_validate_only_workflow_and_integrity(tmp_path):
    package=tmp_path/"D2-VALIDATION-S7-RC-f11f2ab-r3-final"
    ...
    assert manifest["package_id"]=="D2-VALIDATION-S7-RC-f11f2ab-r3-final"
...
def test_packaged_wrapper_fails_closed_on_tampered_artifact(tmp_path):
    package=tmp_path/"D2-VALIDATION-S7-RC-f11f2ab-r3-final"; subprocess.run([sys.executable,str(BUILDER),str(package),"test-commit"],check=True,capture_output=True,text=True)
...
@pytest.mark.skipif(not pwsh(),reason="PowerShell unavailable")
def test_versioned_package_wrapper_enforces_its_own_identity(tmp_path):
    package=tmp_path/"r1"; package_id="D2-VALIDATION-S7-RC-f11f2ab-r1"
    subprocess.run([sys.executable,str(BUILDER),str(package),"test-commit",package_id],check=True,capture_output=True,text=True)
    assert json.loads((package/"D2-package-manifest.json").read_text())["package_id"]==package_id
    sim=tmp_path/"simulation"; simulation(sim)
    result=subprocess.run([pwsh(),"-NoProfile","-File",str(package/"validate_forwarder_s7_rc_f11f2ab.ps1"),"-SimulationRoot",str(sim)],capture_output=True,text=True)
    assert result.returncode==0 and "VALIDATION_RESULT=GO" in result.stdout
