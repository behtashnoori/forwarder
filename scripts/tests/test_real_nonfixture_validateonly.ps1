...
'@
    ...
    $mutations=@{Count=0}
    $topology=[pscustomobject]@{TaskState='Ready';LastTaskResult=0;Enabled=$true;TaskExists=$true;Xml=$xml;Iis=(Join-Path $release 'dist');Owners=@(93244);Runtime=$python;CommandLine=($python+' -m waitress backend.wsgi:app');Health=200;HealthThrows=$false;ActionMismatch='';HealthCalls=0}
    function Reset-Topology {
        ...
        $topology.CommandLine=$python+' -m waitress backend.wsgi:app';$topology.Health=200;$topology.HealthThrows=$false;$topology.ActionMismatch='';$topology.HealthCalls=0
    }
    ...
    Reset-Topology;$topology.Xml=$xml.Replace('cmd.exe','powershell.exe');Assert-Topology 'Wrong_task_execute' 'cmd.exe or python.exe required'
    Reset-Topology;$topology.Xml=$xml.Replace('backend.wsgi:app','unrelated:app');Assert-Topology 'Wrong_task_arguments' 'task must launch the Waitress backend'
    ...
    Reset-Topology;$topology.Xml=$xml.Replace('C:\Windows\System32\cmd.exe',$python).Replace('/d /c &quot;'+$python+'&quot; ','');Assert-Topology 'Direct_python_action'
