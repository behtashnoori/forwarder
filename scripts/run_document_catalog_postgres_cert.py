"""Create, certify, and remove a disposable loopback PostgreSQL database."""
...
database_name = f"forwarder_doc_catalog_cert_{uuid4().hex[:12]}"
...
try:
    ...
    os.environ["DOCUMENT_CATALOG_CERT_DATABASE_URL"] = target_url.render_as_string(hide_password=False)
    runpy.run_path(str(ROOT / "scripts" / "document_catalog_postgres_gate.py"), run_name="__main__")
finally:
    with admin_engine.connect() as connection:
