...
$runtime = Join-Path $runtimeParent "forwarder-dn10-owned-$runId"
...
try {
  ...
  if ($head -ne '20261006_customer_entitlement') { throw "Unexpected migration head: $head" }
  $pgPort = Free-Port
  ...
  python -m pytest backend/tests/test_customer_entitlement_postgresql.py backend/tests/test_phase3_document_context_postgresql.py -q --disable-warnings *> (Join-Path $EvidenceDirectory 'postgresql.log')
  ...
  $journeys = @(@{ name='P306'; seed='document_context'; spec='document-context' })
  ...
  if ((Split-Path -Parent $resolvedRuntime) -ne $resolvedParent -or
      (Split-Path -Leaf $resolvedRuntime) -ne "forwarder-dn10-owned-$runId") {
