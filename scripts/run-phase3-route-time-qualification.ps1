...
$runtime = Join-Path $runtimeParent "forwarder-p310-owned-$runId"
...
try {
  ...
  if ($head -ne '20261009_phase3_route_time') { throw "Unexpected migration head: $head" }
  $pgPort = Free-Port
  ...
  python -m pytest backend/tests/test_phase3_route_time_postgresql.py backend/tests/test_phase3_customer_shipment_postgresql.py backend/tests/test_phase3_cargo_delivery_postgresql.py backend/tests/test_phase3_reported_facts_postgresql.py backend/tests/test_customer_entitlement_postgresql.py backend/tests/test_phase3_document_context_postgresql.py -q --disable-warnings *> (Join-Path $EvidenceDirectory 'postgresql.log')
  ...
  $journeys = @(@{ name='P310'; seed='route_time'; spec='route-time' })
  if ($IncludeRegressions) {
    $journeys += @(
  ...
  if ((Split-Path -Parent $resolvedRuntime) -ne $resolvedParent -or
      (Split-Path -Leaf $resolvedRuntime) -ne "forwarder-p310-owned-$runId") {
