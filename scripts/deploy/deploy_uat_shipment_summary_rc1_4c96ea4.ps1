...
param(
    ...
    [string]$BaselinePath,
    [string]$PreviousBackendReleasePath,
    [string]$PreviousFrontendReleasePath,
...
function Resolve-PreviousReleasePath([string]$Value,[string]$ParameterName) {
    Require (-not [string]::IsNullOrWhiteSpace($Value)) "$ParameterName is required for Production"
    try { $resolved=[IO.Path]::GetFullPath($Value.Trim()) } catch { Fail "$ParameterName is malformed" }
    $expectedRoot=[IO.Path]::GetFullPath($script:ProductionRoot).TrimEnd('\\')
    Require ([string]::Equals((Split-Path -Parent $resolved),$expectedRoot,[StringComparison]::OrdinalIgnoreCase)) "$ParameterName must be a direct child of ProductionRoot"
    Require (Test-Path -LiteralPath $resolved -PathType Container) "$ParameterName does not exist"
    Require (-not [string]::Equals($resolved,$script:TargetRelease,[StringComparison]::OrdinalIgnoreCase)) "$ParameterName equals TargetRelease"
    return $resolved
}
...
function Rollback {
    ...
    try {
        ...
        Restore-TaskState
        Set-IisReference (Join-Path $script:PreviousFrontendRelease 'dist')
        ...
        Assert-IisReference (Join-Path $script:PreviousFrontendRelease 'dist') 'rollback IIS path mismatch'
...
try {
    ...
    Require (Test-Path -LiteralPath $script:PreviousRelease -PathType Container) 'current backend release is absent'
    Require (Test-Path -LiteralPath $script:PreviousFrontendRelease -PathType Container) 'current frontend release is absent'
