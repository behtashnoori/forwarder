...
$Timeouts=@{DB_GATE=15;BACKEND_STOP=30;PORT_RELEASE=30;BACKEND_START=30;LISTENER_VERIFY=30;IIS_VERIFY=15;INTERNAL_HEALTH=15;PUBLIC_HEALTH=20;ROLLBACK_RECOVERY=45}
...
function Create-Target(){if($FixtureStatePath){$Script:Fixture.target_created=$true;Write-Fixture;return};New-Item -ItemType Directory -Path $Script:Target.root -ErrorAction Stop|Out-Null;Get-ChildItem -LiteralPath $ArtifactRoot -Force|Copy-Item -Destination $Script:Target.root -Recurse -Force;Expand-Archive -LiteralPath (Join-Path $Script:Target.root $RuntimeArchiveName) -DestinationPath (Join-Path $Script:Target.root 'runtime') -ErrorAction Stop}
...
function Verify-Health([string]$Kind,[int]$TimeoutSeconds){Assert-NotInjected $Kind;if($FixtureStatePath){Require ([bool]$Script:Fixture.health) ($Kind+' healthy') 'unhealthy';return};$uri=if($Kind -eq 'INTERNAL_HEALTH'){"http://127.0.0.1:$Port/health"}{'https://samand.forwarderet.ir/health'};Invoke-WithTimeout $Kind $TimeoutSeconds {param($HealthUri);$response=Invoke-WebRequest -UseBasicParsing -Uri $HealthUri -TimeoutSec 5;if($response.StatusCode -lt 200 -or $response.StatusCode -ge 300){throw 'unhealthy'}} @($uri)|Out-Null}
