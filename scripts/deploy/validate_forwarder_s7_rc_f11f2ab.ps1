...
try {
    ...
    $args=@('-NoProfile','-File',$entry,'-ValidateOnly','-ArtifactPath',$artifact,'-ManifestPath',$sidecar)
    if($SimulationRoot){$args+=@('-SimulationRoot',$SimulationRoot)}
...
} catch { if($_.Exception.Message -notmatch '^D2 validation package:'){Write-Output 'VALIDATION_RESULT=NO_GO';Write-Output 'PRODUCTION_MUTATION=NO';Write-Output 'DEPLOYMENT_PERFORMED=NO'}; throw }
