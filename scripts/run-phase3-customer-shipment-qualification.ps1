...
$runtime = Join-Path $runtimeParent "forwarder-p309-owned-$runId"
...
try {
  ...
  python -m pytest backend/tests/test_phase3_customer_shipment_postgresql.py backend/tests/test_phase3_cargo_delivery_postgresql.py backend/tests/test_phase3_reported_facts_postgresql.py backend/tests/test_customer_entitlement_postgresql.py backend/tests/test_phase3_document_context_postgresql.py -q --disable-warnings *> (Join-Path $EvidenceDirectory 'postgresql.log')
  ...
  $journeys = @(@{ name='P309'; seed='customer_shipment'; spec='customer-shipment' })
  if ($IncludeRegressions) {
    $journeys += @(
  ...
  if ((Split-Path -Parent $resolvedRuntime) -ne $resolvedParent -or
      (Split-Path -Leaf $resolvedRuntime) -ne "forwarder-p309-owned-$runId") {
