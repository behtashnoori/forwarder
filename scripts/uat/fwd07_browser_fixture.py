...
def main():
    ...
    with app.app_context():
        ...
        definition = DocumentDefinition(code="fwd07-proof", title="مدرک فارسی با نام بلند", is_required=True, allowed_formats='["pdf","png"]', max_file_size_bytes=1048576, max_active_file_count=4, sort_order=1, applicability_scope="all")
        db.session.add_all([org, expert]); db.session.flush(); expert_membership = OperationalMembership(organization_id=org.id, user_id=expert.id, is_active=True, permissions=[]); case.operational_organization_id=org.id; case.assigned_to=expert.id; db.session.add_all([expert_membership, case, definition]); db.session.commit()
