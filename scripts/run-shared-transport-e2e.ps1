...
$modeCount = @($CatalogJourney, $ProductReality, $PersonalAnalytics, $PartyRoleQualification).Where({ $_ }).Count
if ($modeCount -gt 1) { throw 'Qualification modes are mutually exclusive.' }
$purpose = if ($PersonalAnalytics) { 'personal-analytics-e2e' } else { 'shared-transport-e2e' }
$databaseMode = if ($PersonalAnalytics) { 'personal_analytics' } else { 'shared' }
...
function Test-QualificationRuntimeOwnership([System.IO.DirectoryInfo]$Directory, [switch]$AllowLegacy) {
  ...
  if (Test-Path -LiteralPath $manifest -PathType Leaf) {
    try {
      ...
      return $owner.purpose -in @('shared-transport-e2e','personal-analytics-e2e') -and $owner.run_id -eq $Matches[1] -and $owner.repository -eq $root
    } catch { return $false }
  }
...
try {
  ...
  $seed = if ($PersonalAnalytics) { 'scripts\uat\seed_personal_analytics_e2e.py' } elseif ($CatalogJourney) { 'scripts\uat\seed_catalog_journey_e2e.py' } else { 'scripts\uat\seed_shared_transport_e2e.py' }
