...
if ($manifest.git_tag -ne "v1.9.4" -or $manifest.application_version -ne "1.9.4" -or $manifest.database_revision -ne "20260826_org_document_policy" -or -not $manifest.database_migration_included -or $manifest.milestone_type_catalog_apply_status -ne "not applied") { throw "Manifest identity mismatch" }
