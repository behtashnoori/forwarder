...
if ($manifest.git_tag -ne "v1.9.3.1" -or $manifest.application_version -ne "1.9.3.1" -or $manifest.database_revision -ne "20260825_admin_multitenant" -or $manifest.database_migration_included -or $manifest.milestone_type_catalog_apply_status -ne "not applied") { throw "Manifest identity mismatch" }
