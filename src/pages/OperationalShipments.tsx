...
export default function OperationalShipments(){
 ...
 const load=(activeFilters=filters,onlyActive=activeOnly)=>{setLoading(true);setError("");const q=new URLSearchParams({...activeFilters,page:String(page),...(onlyActive?{active:"true"}:{})});listOperationalShipments(q.toString()).then(r=>{setRows(r.data);setMore(r.meta.has_more)}).catch(e=>setError(e instanceof ApiError&&e.status===403?t("operations.forbidden"):e instanceof Error?e.message:t("operations.error"))).finally(()=>setLoading(false));};
 ...
 const applyFilters=(next=filters)=>{if(page===1)load(next,activeOnly);else setPage(1);};
