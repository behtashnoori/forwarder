...
export default function CustomerManagement() {
  ...
  return <main dir="rtl" className="min-h-screen bg-slate-50 p-4 sm:p-8"><div className="mx-auto max-w-5xl space-y-5">
    ...
    {!rows.length ? <Card><CardContent className="py-12 text-center"><Users className="mx-auto mb-3 h-10 w-10 text-muted-foreground" /><p>مشتری‌ای در این سازمان یافت نشد.</p></CardContent></Card> : <section className="grid gap-3">{rows.map((customer) => <Card key={customer.id}><CardHeader className="py-4"><CardTitle className="text-base">{customer.company_name || customer.name}</CardTitle>{customer.name && <CardDescription>{customer.name}</CardDescription>}</CardHeader><CardContent className="pt-0 text-sm text-muted-foreground">{customer.status === "active" ? "فعال" : "غیرفعال"}</CardContent></Card>)}</section>}
    <Sheet open={open} onOpenChange={(value) => { setOpen(value); if (!value && !create.isPending) { setForm(emptyForm); setDuplicateAcknowledged(false); setDuplicateMessage(""); create.reset(); } }}><SheetContent side="left" className="overflow-y-auto" dir="rtl"><SheetHeader><SheetTitle>ایجاد مشتری</SheetTitle><SheetDescription>مشتری فقط در سازمان فعال شما ثبت می‌شود.</SheetDescription></SheetHeader><form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}>
      ...
      <CustomerField label="نام مخاطب" value={form.first_name} onChange={(first_name) => setForm({ ...form, first_name })} />
      <CustomerField label="نام خانوادگی مخاطب" value={form.last_name} onChange={(last_name) => setForm({ ...form, last_name })} />
