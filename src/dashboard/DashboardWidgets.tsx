...
export function DashboardWidget({ widget, result, notApplicableFilters = [] }: { widget: WidgetDefinition; result: WidgetResult; notApplicableFilters?: string[] }) {
  const response = result.response;
  const metric = widget.query.metric_keys?.[0] || "";
  const dimension = widget.query.dimension_keys?.[0] || "";
  const [drilldown, setDrilldown] = useState<AnalyticsDrilldownResponse>();
  const chartData = useMemo(() => widget.query.query_kind === "ROWSET" ? [] : (response?.rows || []).map((row) => ({ label: String(row[dimension] ?? "ثبت نشده"), value: (row[metric] as { value?: number })?.value ?? null })), [response, widget.query.query_kind, dimension, metric]);
