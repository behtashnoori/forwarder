...
const metric = (key:string, dimensions:string[] = []) => ({ metric_key:key,business_name:key,grain:"fact",unit:"count",readiness:"READY" as const,supported_dimensions:dimensions,supported_filters:["TIME","CUSTOMER","PROJECT"],supported_time_dimensions:["created"],supported_time_grains:["day"],drilldown_type:"shipment_list",drilldown_supported:true,semantic_version:"analytics-semantic-v1" });
...
describe("Dashboard definition validator", () => {
  ...
  it("keeps the uncertified Delay Trend time-segment drilldown disabled", () => {
    ...
    expect(delayTrend?.query).toMatchObject({ metric_keys:["DELAY_CASE_COUNT"], dimension_keys:["TIME"], time_dimension:"created", time_grain:"day" });
