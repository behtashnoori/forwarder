...
describe("ReleaseIdentity", () => {
  ...
  it("always renders the compile-time label and exposes sanitized support details", async () => {
    vi.mocked(api.getReleaseIdentity).mockResolvedValue({projection:"support",data:{application_version:"1.9.5.1",backend_version:"1.9.5.1",release_tag:"v1.9.5.1",short_commit:"1234567890ab",database_revision:"head"}});
    render(<ReleaseIdentity details />);
    expect(screen.getByText("Forwarder 1.9.5.1")).toBeInTheDocument();
