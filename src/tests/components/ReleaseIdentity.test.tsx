...
describe("ReleaseIdentity", () => {
  ...
  it("always renders the compile-time label and exposes sanitized support details", async () => {
    vi.mocked(api.getReleaseIdentity).mockResolvedValue({projection:"support",data:{application_version:"1.10.0",backend_version:"1.10.0",release_tag:"forwarder-uat-v1.10.0",short_commit:"1234567890ab",database_revision:"head"}});
    render(<ReleaseIdentity details />);
    expect(screen.getByText("Forwarder 1.10.0")).toBeInTheDocument();
