...
describe("operational shipment saved-view adapter",()=>{
 it("roundtrips the governed ROWSET population contract",()=>{const definition=toSavedViewDefinition(filters);expect(applySavedViewDefinition(definition,{...filters,status:"",date_from:"",date_to:""})).toEqual(filters);expect(definition.query_definition.query_kind).toBe("ROWSET");expect(definition.query_definition.population).toBe("SHIPMENTS");});
