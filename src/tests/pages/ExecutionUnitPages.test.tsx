...
describe("Release 1.2.0 execution unit pages",()=>{
 ...
 it("renders the expert compact list and lazy-loads timeline only after open",async()=>{render(<MemoryRouter initialEntries={["/operations/projects/project/units"]}><Routes><Route path="/operations/projects/:projectId/units" element={<ExecutionUnits/>}/></Routes></MemoryRouter>);expect(await screen.findByText("U-0001")).toBeInTheDocument();expect(api.getExecutionUnitTimeline).not.toHaveBeenCalled();await userEvent.click(screen.getByRole("button",{name:"Open"}));await waitFor(()=>expect(api.getExecutionUnitTimeline).toHaveBeenCalledWith("project","opaque-unit"));expect(screen.getByText("No updates.")).toBeInTheDocument()});
