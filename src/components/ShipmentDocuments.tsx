...
export default function ShipmentDocuments({ shipmentPublicId }: { shipmentPublicId: string }) {
  ...
  const upload = async () => {
    ...
    try {
      ...
      await uploadShipmentDocument(shipmentPublicId, form, crypto.randomUUID());
      ...
      <CardContent className="space-y-4">
        ...
        {canManage ? (
          <div className="grid gap-2 rounded border p-3 sm:grid-cols-2">
            ...
            <Input className="sm:col-span-2" aria-label="انتخاب فایل سند" type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,.docx,.xlsx" onChange={(event) => setFile(event.target.files?.[0] || null)} />
            <Button className="sm:col-span-2" disabled={busy || !file || !title.trim()} onClick={() => void upload()}><Upload className="ms-2 h-4 w-4" />{busy ? "در حال بارگذاری…" : "بارگذاری سند برای محموله"}</Button>
          </div>
        ) : (
