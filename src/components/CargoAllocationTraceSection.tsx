...
export default function CargoAllocationTraceSection({ shipmentId, planId, closed = false }: { shipmentId: string; planId: number; closed?: boolean }) {
  ...
  const dimension = closed ? "ACTUAL" : allocation.dimension;
  ...
  const current = currentStages.find((execution) => execution.stage_execution_public_id === allocation.stageId)?.allocations.find((row) => row.dimension === dimension);
  ...
  const save = async (quantity = allocation.quantity) => {
    ...
    const occurred = closed ? localDateTimeInputToUtc(occurredAt) : null;
    if(closed && (!occurred || !allocation.reason.trim())) {setError("زمان واقعیت پیشین و دلیل اصلاح را وارد کنید.");return;}
    const payload = { dimension, ...(occurred ? {occurred_at:occurred} : {}), quantity, expected_version: current?.version || 0, ...(allocation.reason.trim() ? { reason: allocation.reason.trim() } : {}) };
