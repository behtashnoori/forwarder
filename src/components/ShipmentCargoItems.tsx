...
export default function ShipmentCargoItems({ shipmentPublicId, projectPublicId, legacyDescription, stageScoped = false, closed = false }: { shipmentPublicId: string; projectPublicId?: string | null; legacyDescription?: string | null; stageScoped?: boolean; closed?: boolean }) {
  ...
  const saveCargo = async (item: ShipmentCargoItem) => {
    ...
    try {
      const saved = await updateShipmentCargoItem(shipmentPublicId, item.public_id, {
        ...
        ...(closed ? {} : {planned_quantity: edit.planned_quantity}),
  ...
  return <>
    <Card dir="rtl"><CardHeader><CardTitle>کالا، مشتری و درخواست منبع</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      {!closed && <details><summary className="cursor-pointer font-medium">افزودن ردیف کالا</summary><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {!items.length ? <p className="text-sm text-slate-600">هنوز کالایی برای این محموله ثبت نشده است.</p> : <div className="grid gap-3 lg:grid-cols-2">{items.map((item) => <article className="rounded border p-3 text-sm" key={item.public_id}>
        ...
        <details className="mt-3"><summary className="cursor-pointer font-medium">تکمیل یا اصلاح اطلاعات</summary><fieldset disabled={savingCargo[item.public_id]} aria-busy={savingCargo[item.public_id] || undefined} className="mt-2 grid gap-2 sm:grid-cols-2">
          ...
          <Input disabled={closed} aria-label={`Edit planned quantity line ${item.line_number}`} type="number" min="0.000001" step="any" placeholder="برنامه‌ریزی‌شده" value={editFor(item).planned_quantity} onChange={(event) => changeEdit(item, { planned_quantity: event.target.value })} />
          ...
          <Input aria-label={`Edit cargo reason line ${item.line_number}`} placeholder={closed?"دلیل اصلاح سابقه (الزامی)":"دلیل اصلاح برنامه یا مقدار واقعی، در صورت وجود"} value={editFor(item).reason} onChange={(event) => changeEdit(item, { reason: event.target.value })} />
