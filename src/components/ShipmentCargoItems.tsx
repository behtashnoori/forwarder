  ...
  return <>
    <Card dir="rtl"><CardHeader><CardTitle>کالا، مشتری و درخواست منبع</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      {!items.length ? <p className="text-sm text-slate-600">هنوز کالایی برای این محموله ثبت نشده است.</p> : <div className="grid gap-3 lg:grid-cols-2">{items.map((item) => <article className="rounded border p-3 text-sm" key={item.public_id}>
        ...
        <p>بسته‌بندی: {item.packaging?.fa_name || "نامشخص"} · HS: {item.hs_code_snapshot || "نامشخص"}</p><p>وزن ناخالص: {item.gross_weight ? `${formatQuantity(item.gross_weight.value)} ${item.gross_weight.uom_symbol}` : "نامشخص"} · حجم: {item.volume ? `${formatQuantity(item.volume.value)} ${item.volume.uom_symbol}` : "نامشخص"}</p>{item.destination_description && <p>مقصد قلم: {item.destination_description}</p>}<p>تخصیص‌یافته: {formatQuantity(item.allocated_quantity)} {item.uom_symbol_snapshot} · باقیمانده: {formatQuantity(item.remaining_quantity ?? item.quantity)} {item.uom_symbol_snapshot}</p>{item.description_snapshot && <p className="mt-2 text-slate-600">{item.description_snapshot}</p>}
