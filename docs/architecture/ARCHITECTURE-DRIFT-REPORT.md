...
| Locations | `TrackingLocationReference` feeds the expert tracking selector | `LogisticsPoint` is governed master data; `CanonicalLocation` is the route bridge/snapshot identity | Do not silently make either catalog authoritative for the other. Mapping/convergence requires an ADR and migration plan. |
...
- The expert tracking selector does not consume `LogisticsPoint`; silently switching it would change authority, tenancy, history, and API semantics.
