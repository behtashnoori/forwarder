...
| Public Request tracking requires an opaque non-enumerable capability and minimized public projection | Runtime accepts numeric `ShipmentRequest.id`, issues weak/fallback codes, and exposes internal/private fields | P0 IDOR/enumeration and over-disclosure release blocker | Implement ADR-052; reject numeric/legacy authority; issue only `SR2-` capabilities; enforce exact allowlist | MT-3 public tracking security closure | Product/Architecture/Security | REFERENCE RESOLVED — IMPLEMENTATION OPEN | ADR-052; MT-0 contract; MT-3 plan; failed Final Product Acceptance finding |

All rows have `REFERENCE_IMPACT=UPDATE_REQUIRED`. Their project reference updates are `CLOSED_FOR_REFERENCE_PHASE`; Documents is specifically `CLOSED_FOR_DOCUMENTS_DESIGN`. Runtime/product status remains as shown above, and no reference closure is an implementation PASS.
