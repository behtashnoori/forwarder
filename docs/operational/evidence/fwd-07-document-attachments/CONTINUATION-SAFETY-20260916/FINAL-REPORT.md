...
## Observed remote dependency/checkout continuation

Quality run 35139682902 at 3b92f4d004aebc2638c918e04b618418eb083b76 started its run-owned PostgreSQL, ran the suite and performed owned shutdown. Backend FAILED: 38 failed / 992 passed / 214 skipped / 1 xfailed / 80 errors, 357.85s. Frontend/structure and separate secret run 35139683024 PASSED. The earlier startup correction is therefore remotely verified; this failure is not hidden.

Root causes: requirements allowed an unreviewed newer PyJWT while the existing ADR-047 signer explicitly permits only 2.13.0; Linux Git checkout used LF while the existing historical-package verifier requires the reviewed migration's exact CRLF SHA256. Corrective commit 9196663 pins PyJWT==2.13.0 without changing the signer gate, and adds only an explicit eol=crlf checkout attribute for that one historical migration. Migration Git contents, reviewed raw hash, credential fingerprint, package declaration and scanner exception policy remain unchanged. The Git checkout filter reproduces 6ed41e455ed80e69922f201dbe2e8fd4e9db3e1c60f49bf64fb39a4451013554 exactly from existing index bytes; no new scanner waiver/exception is introduced.

Owned bounded run 3743ef8a928a429697934826ef751731 PASSED: four known gates plus package-secret/config/credential tests, 34 passed; focused regression 4 passed; eight negative boundary cases and positive owned PostgreSQL proof passed; owned shutdown. New diagnostics remain outside Git except redacted selected excerpt and lossless transport. Exact full remote log remains in the owned local diagnostic root. Raw excerpt original remains unchanged/untracked.

Quality run 35141115844 and secret run 35141115831 are the observed workflows for corrective commit 9196663; final results and exact delivery-head results will be printed in the final response after observation. Supersedes the earlier NOT_OBSERVED remote status only; no local result is relabeled remote. Worktree is deliberately NOT_CLEAN because ten exact raw evidence originals remain untracked; earlier clean/ref wording is preparation history, not the final status.
