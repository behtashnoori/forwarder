...
$workspace = 'D:\1-webapp\forwarder-dev\phase3-p3-12-closure'
$pgBin = 'C:\Program Files\PostgreSQL\18\bin'
$runId = [guid]::NewGuid().ToString('N')
$runtimeParent = [System.IO.Path]::GetTempPath().TrimEnd([System.IO.Path]::DirectorySeparatorChar)
$runtime = Join-Path $runtimeParent "forwarder-p312-owned-$runId"
...
try {
  ...
  if ($dirty -or $productHead -ne 'd1574fa137017ac41331cf039117ab9c19e101a3') { throw 'Unexpected retry source' }
  $head = (python -m scripts.browser_migration_contract repository-head).Trim()
  if ($head -ne '20261010_phase3_closure') { throw "Unexpected migration head: $head" }
  $pgPort = Free-Port
  ...
  $journeys = @(@{ name='P312'; seed='closure'; spec='closure' })
  if ($IncludeRegressions) {
    $journeys += @(
  ...
  $journeys = @($journeys | Where-Object { $_.name -eq 'MT3' })
  ...
  if ((Split-Path -Parent $resolvedRuntime) -ne $resolvedParent -or
      (Split-Path -Leaf $resolvedRuntime) -ne "forwarder-p312-owned-$runId") {
