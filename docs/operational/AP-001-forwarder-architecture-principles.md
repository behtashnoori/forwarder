...
| 3 | **Reference Data is administrator-managed, never deployment-seeded.** | Stable classifications require stewardship, not hidden installation data. | The application installs and operates with empty catalogs; authorized administrators create records through Admin UI. Seed/import/export tooling is optional and no release depends on population. | Only indispensable System Data may be installer-generated, and it must not be mislabeled Reference Data. |
