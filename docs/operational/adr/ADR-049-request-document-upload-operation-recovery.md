...
Before network send, the authenticated UI generates a cryptographically random UUID v4 per selected file, canonical lowercase 36-character form, validates version/variant, computes SHA256/length, and durably records the pending manifest before sending. Scope: trusted tenant, authorized request, requirement or miscellaneous discriminator, APPEND/REPLACE, explicit replaced-file identity, and initiating authenticated actor. Server uniqueness is `(organization_id, operation_uuid)`; reusing a tenant key with another target/action/actor conflicts after authorization. The public requirement selector is `(request public UUID, source DocumentDefinition public UUID, frozen source_definition_revision)`: the existing request/source/revision uniqueness resolves exactly one CaseDocumentRequirement. The authorized document-owner projection supplies that tuple; internal numeric requirement IDs remain persistence/compatibility handles, never new public recovery authority. No nonexistent requirement public UUID or new requirement-identity backfill is assumed. Missing/ambiguous/quarantined references fail closed. Cross-tenant queries are non-disclosing. Collision requires a fresh deliberate operation only after establishing that this unsent identity collided; never rotate an unknown sent identity automatically.
...
Receipt consists of immutable operation/scope/hash/result/completion fields in this same row, atomically written with file/audit. Add nullable unique `CaseDocumentFile.upload_operation_id` reference; enforce reciprocal result linkage at finalize and PostgreSQL scoped composite FKs for request/requirement/replacement/result ownership, adding parent composite uniqueness where needed without changing identities. The frozen public requirement selector uses existing definition public UUID and request/source/revision uniqueness; no requirement identity column is added. Actor deletion leaves attribution unknown and prevents actor replay; never infer a replacement actor. Index request/actor/created_at for bounded authorized discovery and state/lease for reconciliation. Constraints must preserve quarantine/legacy-null envelopes of existing tables.
...
- Proposed edition 2: clarified the existing requirement selector (CaseDocumentRequirement has no public UUID) and miscellaneous APPEND-only behavior after model review. Prior named edition SHA256 14B4B114BE9D4ACB9CC22DC4F30DF42EBD6375083F99D86FBB98DDE738016AD2 remains in Git history; neither edition is Accepted.
