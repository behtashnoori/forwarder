...
- 2026-08-23: PHASE 4A IMPLEMENTED LOCALLY — explicit idempotent Organization-Admin materialization creates an ordinary tenant LogisticsPoint with immutable adoption/global provenance, same-tenant and one-per-adoption database constraints, one-time upstream seeding, and no automatic synchronization; existing-point linking, Expert-specific integration, project/tracking changes, seed, Production access, deployment and push remain unauthorized.
- 2026-08-23: PHASE 4B CERTIFIED LOCALLY — materialized and organization-only tenant LogisticsPoints share the existing Expert tracking selector, tracking FK/snapshot, project selection and read contracts; tenant/type activity and permission checks remain authoritative, while upstream global deprecation and adoption deactivation do not remove the tenant point. Project selection now consistently rejects inactive point types. No operational GlobalLogisticsPoint API/FK, migration, seed, Production access, deployment or push was introduced.
- 2026-08-23: CATALOG V1 REVIEW PACKAGE PREPARED LOCALLY — deterministic China→Iran candidate data, authoritative-source manifest, owner decision register and all-64 legacy reconciliation prepared for review; every simulated service-layer row remains DRAFT/UNVERIFIED and no Production seed, API mutation, adoption, materialization, migration, deployment or push was performed.
