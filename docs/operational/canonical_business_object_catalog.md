...
- References: original ADR-001 through ADR-020 and PDR-001 through PDR-011 baseline, plus accepted scoped extensions PDR-017, PDR-019, PDR-020, ADR-047, and ADR-050
...
| Business Concept | Canonical Name | Business Definition | Technical Definition | Aggregate Owner | Primary Identifier | Lifecycle Owner | Parent Object | Child Objects | Visible To | Deprecated Aliases | Future Compatibility Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Document artifact | `DocumentArtifact` | One immutable physical file version and its intrinsic technical metadata | Private stored binary metadata: checksum, media type, size, storage key, uploader, signature metadata | Document boundary | Opaque `document_artifact_id`; checksum is integrity evidence, not identity | System-owned immutable history; retention/purge policy only | Organization/document store | Exact-version Attachments and version lineage as modeled | Only through an authorized DocumentAttachment or privileged storage operation | File, CaseDocumentFile, UploadedFile | Existing CaseDocumentFile may bridge to artifact/version; storage key is never public identity; owning Expert initiates management but does not own history |
| Document attachment | `DocumentAttachment` | Contextual link between an exact DocumentArtifact/DocumentVersion and one business scope or logical requirement | Link containing exact version, parent scope, type, visibility, classification, approval, tags, and validity | Parent scope/document boundary | Opaque `document_attachment_id` | Document attachment/version policy | Shipment/Case or other separately accepted parent scope | Approvals, Assessments, AuditEntries | Audiences allowed by a separate read contract; management is owning-Expert-only | Attachment, Document, LinkedFile | Multiple current files may serve one logical requirement; association never grants standalone management authority |
| Document version | `DocumentVersion` | Immutable revision in one targeted physical-file lineage | Version record linking artifact/attachment lineage and explicit supersession | DocumentAttachment/document boundary | Opaque `document_version_id` + lineage version number | System-owned replacement/supersession policy | DocumentAttachment or logical file lineage | Approvals, verification evidence | Same as authorized attachment/version read policy | FileVersion, ReplacementFile | Existing CaseDocumentFile `version_number`/`superseded_by` map additively; old versions remain auditable and do not count as additional current readiness evidence |
| Requirement | `DocumentRequirement` | Versioned logical policy snapshot defining required/optional evidence and validation limits for a parent scope | Existing DocumentDefinition/CaseDocumentRequirement/OperationalDocumentRequirement concepts separated into definition and applied requirement | Parent scope/document policy owner | Opaque `document_requirement_id`; definition code/revision | Requirement policy; satisfaction/readiness is derived | ShipmentRequest or OperationalShipment under the governed slice | Current and historical DocumentAttachments/Versions | Owning Expert and separately authorized read-only oversight; no Customer management | Requirement, CaseDocumentRequirement, DocumentDefinition when conflated | Use `DocumentDefinition` for reusable policy, `DocumentRequirement` for applied snapshot, and readiness for the derived result; file quantity is not readiness |
...
| Tracking code | `TrackingCode` | High-entropy public tracking credential or reference; revocation is a target lifecycle capability | ADR-052 Request scope currently uses a versioned 128-bit plaintext bearer in the unique legacy column; protected/hashed storage with issue, rotation, revocation, scope, and audit metadata is the target | Scope identity/access policy | The protected code; never sequential/derived | Tracking-access governance | Project or legacy ShipmentRequest | Public projection access record | Intended customer/share recipients | TrackingNumber, PublicCode, Request ID | Request `SR2-` authority is implemented without hash/rotation/revocation; future Project TrackingCode remains separate from legacy request code and ProjectCode |
