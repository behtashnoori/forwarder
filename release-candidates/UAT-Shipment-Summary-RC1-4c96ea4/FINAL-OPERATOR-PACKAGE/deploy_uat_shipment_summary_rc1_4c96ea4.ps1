...
$CandidateId = 'UAT-Shipment-Summary-RC1-4c96ea4'
$SourceCommit = '4c96ea4e476767d4f6ab67e75fc21f3fed17f774'
$ArtifactName = 'Forwarder-UAT-Shipment-Summary-RC1-4c96ea4-4c96ea4.zip'
$ArtifactHash = 'f4de1313926a561a687f744b364b23bac119aecb91cb34e594ce3f421e482399'
$ManifestHash = '06fa602baa6ee46e65fc54ff90d27939e3feaee72e34ed71492cf859293fed34'
$RuntimeArtifactName = 'Forwarder-Windows-Runtime-UAT-Shipment-Summary-RC1-4c96ea4.zip'
$RuntimeArtifactHash = 'f4a8f108aa89a78d7986f01fb8f6aa8af5e2d35e00617a8453eb1f15df945070'
$RuntimeManifestHash = '231b44341842d80fbad107ec2feec73a839dc1b67d9b119c9474e8028944e0c2'
$TargetHead = '20260909_cargo_transport_allocation'
...
try {
    ...
    if($SimulationRoot -or $QualificationRoot){ $fixtureRoot=if($QualificationRoot){$QualificationRoot}else{$SimulationRoot}; Require (Test-Path -LiteralPath $fixtureRoot -PathType Container) 'fixture root is absent'; $script:ProductionRoot=Join-Path $fixtureRoot 'production'; $script:RuntimeRoot=Join-Path $fixtureRoot 'runtime'; $script:StagingRoot=Join-Path $fixtureRoot 'staging'; $script:PreviousRelease=Join-Path $script:ProductionRoot 'release-adcc5da-adr043'; $script:TargetRelease=Join-Path $script:ProductionRoot 'release-UAT-Shipment-Summary-RC1-4c96ea4'; $script:ProductionEnv=Join-Path $script:RuntimeRoot 'production.env'; $ArtifactPath=Join-Path $script:StagingRoot $ArtifactName; $ManifestPath="$ArtifactPath.manifest.json"; if(-not $RuntimeArtifactPath){$RuntimeArtifactPath=Join-Path $script:StagingRoot $RuntimeArtifactName}; if(-not $RuntimeManifestPath){$RuntimeManifestPath="$RuntimeArtifactPath.manifest.json"} }
    else { Require ([Environment]::MachineName -eq $ExpectedHost) 'wrong host'; Require (([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) 'Administrator is required'; $script:ProductionRoot='C:\1-webapp\forwarder-production'; $script:RuntimeRoot='C:\1-webapp\forwarder-runtime'; Require (-not [string]::IsNullOrWhiteSpace($PreviousReleasePath)) 'PreviousReleasePath is required for Production'; try { $resolvedPrevious=[IO.Path]::GetFullPath($PreviousReleasePath.Trim()) } catch { Fail 'PreviousReleasePath is malformed' }; $expectedProductionRoot=[IO.Path]::GetFullPath($script:ProductionRoot).TrimEnd('\'); Require ([string]::Equals((Split-Path -Parent $resolvedPrevious),$expectedProductionRoot,[StringComparison]::OrdinalIgnoreCase)) 'PreviousReleasePath must be a direct child of ProductionRoot'; Require (Test-Path -LiteralPath $resolvedPrevious -PathType Container) 'PreviousReleasePath does not exist'; $script:PreviousRelease=$resolvedPrevious; $script:TargetRelease=Join-Path $script:ProductionRoot 'release-UAT-Shipment-Summary-RC1-4c96ea4'; Require (-not [string]::Equals($script:PreviousRelease,$script:TargetRelease,[StringComparison]::OrdinalIgnoreCase)) 'PreviousReleasePath equals TargetRelease'; $script:ProductionEnv=Join-Path $script:RuntimeRoot 'production.env' }
