...
if ($manifest.application_version -ne "1.9.5.1" -or $manifest.previous_version -ne "1.9.5") { throw "Version mismatch" }
if ($manifest.git_tag -ne "v1.9.5.1" -or $manifest.git_commit -ne $manifest.backend_revision -or -not $manifest.git_tree -or -not $manifest.git_tag_object) { throw "Git identity mismatch" }
$expectedRevisions = @("20260828_referral_state_compat")
if ($manifest.database_revision -ne "20260828_referral_state_compat" -or $manifest.production_baseline_revision -ne "20260827_org_hostname" -or $manifest.previous_database_revision -ne "20260827_org_hostname" -or -not $manifest.database_migration_included) { throw "Database metadata mismatch" }
if ((@($manifest.upgrade_revisions) -join "|") -ne ($expectedRevisions -join "|")) { throw "Migration path mismatch" }
if ($manifest.rollback_release -ne "v1.9.5" -or $null -ne $manifest.rollback_restore_required_from_revision) { throw "Rollback metadata mismatch" }
