...
if ($manifest.application_version -ne "1.7.0" -or $manifest.previous_version -ne "1.6.1") { throw "Version mismatch" }
if ($manifest.git_tag -ne "v1.7.0" -or $manifest.git_commit -ne $manifest.backend_revision) { throw "Git identity mismatch" }
if ($manifest.database_revision -ne "20260810_logistics_network" -or -not $manifest.database_migration_included) { throw "Database metadata mismatch" }
...
$required = @("index.html", $manifest.frontend_entry_js, $manifest.frontend_entry_css, "web.config", "manage.py", "requirements.txt", "DEPLOYMENT.md", "SMOKE-TEST.md", "ROLLBACK.md", "MIGRATION-PREFLIGHT.md", "VERIFY-SERVER.ps1", "backend/migrations/versions/20260810_logistics_network.py")
...
$seedAutoRun = Get-ChildItem -LiteralPath $PSScriptRoot -Recurse -File -Include *.ps1,*.bat,*.cmd,*.yml,*.yaml | Select-String -Pattern '(?i)(reference.*seed.*apply|seed.*--apply)' -List -ErrorAction SilentlyContinue | Where-Object { $_.Path -notlike '*MIGRATION-PREFLIGHT.md' }
